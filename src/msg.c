...
__regparm3 void NO_SANITIZE_FLOAT_CAST_OVERFLOW MSG_WriteDeltaField(struct snapshotInfo_s *snapInfo, msg_t *msg, const int time, const byte *from, const byte *to, const struct netField_s* field, int fieldNum, byte forceSend)
{
	int nullfield;
	int32_t timetodata;
	int32_t absbits;
	...
	int32todata = (int)*(uint32_t* )todata;
...
int MSG_ReadDeltaGroundEntity(msg_t *msg)
{
  ...
  for ( j = 2; j < 10; j += 8 )
  {
    value |= ((unsigned)MSG_ReadByte(msg)) << j;
  }
  return value;
}
...
void MSG_ReadDeltaField(msg_t *msg, int time, const void *from, const void *to, netField_t *field, int print, bool noXor)
{
  ...
  if ( !bits )
  {
    if ( !MSG_ReadBit(msg) )
    {
      *(uint32_t*)toF = ((unsigned)MSG_ReadBit(msg)) << 31;
      return;
    }
    if ( !MSG_ReadBit(msg) )
    {
  ...
  switch(field->bits)
  {
	...
	default:
		...
		{
			t |= (((unsigned)MSG_ReadByte(msg)) << bit_vect);
		}
		if ( bits == 32 )
		{
			bit_vect = -1;
